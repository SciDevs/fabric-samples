...
function upgradeNetwork() {
  docker inspect -f '{{.Config.Volumes}}' orderer.dmail.io | grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.dmail.io
  docker cp -a orderer.dmail.io:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.dmail.io
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.dmail.io

  for PEER in peer0.org1.dmail.io peer1.org1.dmail.io peer0.org2.dmail.io peer1.org2.dmail.io; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.dmail.io/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml 
  cd crypto-config/peerOrganizations/org2.dmail.io/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.dmail.io`` & ``peer0.org2.dmail.io``).  Secondly, we point to
